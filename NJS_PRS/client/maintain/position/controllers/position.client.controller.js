...
angular.module('position').controller('PositionController', PositionController);
    ...
    function PositionController($scope, $http, uiGridConstants,$uibModal, i18nService, instance, toaster,
        ...
        var positionIntroductionGridColumnDefs = [
            ...
            {field: 'count', allowCellFocus:false, displayName:'招聘人数'},
            {field: 'city',allowCellFocus:false, displayName:'工作城市'},
            {field: 'location',allowCellFocus:false, displayName:'工作地址', visible:false},
            ...
            {field: 'negotiable', allowCellFocus:false, displayName:'是否面议'},
            {field: 'certificate', allowCellFocus:false, displayName:'学历'},
        ...
        $scope.publishJob = function(){
            var userInfo = JSON.parse($cookies.get('USER_INFO'));
            if(userInfo.role === '普通') toaster.pop('error', '你没有发布的权限');
            else if($scope.gridApi.grid.selection.selectedCount == 0){
                toaster.pop('error', TIP_NO_DATA_SELECT);
            }else {
                $.confirm({
                    title: '确定发布所有选中的职位？',
        ...
        $scope.stopPublish = function(){
            var userInfo = JSON.parse($cookies.get('USER_INFO'));
            if(userInfo.role === '普通') toaster.pop('error', '你没有权限');
            else if($scope.gridApi.grid.selection.selectedCount == 0){
                toaster.pop('error', TIP_NO_DATA_SELECT);
            }else {
                $.confirm({
        ...
        $scope.batchInsert = function () {
            ...
            instance.allPositionTypes = $scope.allPositionTypes;
            instance.allCompanies = $scope.workAddrs;
            $uibModal.open({
                templateUrl: 'client/maintain/position/views/batchInsert.html',
                controller: 'PositionBatchInsert',
                size: 'lg',
                backdrop: 'static'
            }).result.then(function (result) {
                if (["create", "save"].indexOf(result) > -1)
                    $scope.myDataGrid.data.push(positions);
            });
        }
