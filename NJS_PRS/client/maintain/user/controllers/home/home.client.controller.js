...
angular.module('home').controller('homeController',['$scope','$http','$cookies','ApplicantService','PositionService',
    function ($scope, $http,$cookies,ApplicantService,PositionService) {

    // var getApplicants = $http.get('/api/v1/applicant',{params: {sort: 'updated'}});
    // var getActivePositions = $http.get('/api/v1/position', {params: {query: {status: '已发布'}}});
    var getApplicants, getActivePositions;
    ...
    function formatApplicants() {
        ...
        applicants.forEach(function (applicant) {
            ...
            applicant.applied.forEach(function (position) {
    ...
    $scope.init = function () {
        ...
        ApplicantService.getApplicants({'applied.0': {$exists: true}}, function(err, res){
            if(res.data) {
                getApplicants = res.data;
                setScope(res.data);
                formatApplicants();
                randerChart();
            }
        });
        PositionService.getPositions({status: '已发布'}, function(err, res){
            if(res.data){
                $scope.activePositions = res.data.map(function (d) {
                    return d.name;
                });
            }
        })
