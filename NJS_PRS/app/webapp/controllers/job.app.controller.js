...
function applyShow(req, res, next){
    var applicant = req.session.applicant,
        ...
        applies = _.orderBy(applicant.applied, ['position.updated'], ['desc']);
    applies = _.filter(applies, function(a){return a.position});
    if(!_.isEmpty(applicant) && !_.isEmpty(applies)){
        async.waterfall(
            [
                function(callback){
        ...
        res.render('./app/webapp/views/apply', {applicant: applicant});
    }
}
...
function collectShow(req, res, next){
    ...
    if(!_.isEmpty(applicant) && !_.isEmpty(collects)){
                    ...
                    _.forEach(collects, function(position, i){
                        (function(position, i){
                            companyInfoService.getCompanyInfos({type:position.position.workAddr},function(err, companyInfos) {
                                ...
                                position.applied = !_.isEmpty(applicant)?_.indexOf(_.map(applies,
                                    function(p){return p.position._id}),position.position._id):-1;
                                position.position.updated = moment(position.position.updated).format('YYYY-MM-DD HH:mm');
        ...
        res.render('./app/webapp/views/collect', {applicant: applicant});
    }
}
        ...
        [
            function(callback) {
                _.remove(applicant.collected, function (obj) {
                    return obj.position && obj.position._id == _id
                });
                applicantService.create(applicant, function (err, result) {
                    if (err) {
                        return callback(err);
